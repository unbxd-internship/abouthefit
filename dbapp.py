...
#insert products one at a time into the database
...
#get names of categories and sub categories to populate the dropdowns       
...
def category():
        try:
                ...
                return Response(json.dumps({'count': result_count, 'products': temp}), status = 200, mimetype='application/json')
...
def search():
        ...
        try:
                ...
                final_response = {'products': [], 'numberOfProducts': 0}
                for i in response['products']:
                        final_response['products'].append({'sku': i['sku'], 'title': i['title'], 'productdescription': i['description'], 'price': i['price'], 'productimage': i['productImage'], 'catlevel1name': i['catlevel1Name'], 'catlevel2name': i['catlevel2Name']})
                final_response['numberOfProducts'] = response['numberOfProducts']
